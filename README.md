...
이 프로젝트는 [Vercel](https://vercel.com/)을 통해 아주 쉽게 배포할 수 있습니다. 이미 설정된 자동화 스크립트 덕분에 엑셀 데이터 변환도 배포 과정에서 자동으로 처리됩니다.

### 1. GitHub 저장소 준비
- 현재 로컬의 변경사항을 GitHub에 모두 push 합니다.

### 2. Vercel에서 프로젝트 연결
1. Vercel 대시보드에서 **[+ New Project]** 버튼을 클릭합니다.
2. 이 프로젝트의 GitHub 저장소를 선택하여 **[Import]** 합니다.
3. 설정 창에서 다음을 확인합니다:
    - **Framework Preset**: `Vite`
    - **Build Command**: `node import_data.mjs && vite build` (또는 기본값으로 둡니다. `package.json`에 이미 설정되어 있습니다.)
4. **[Deploy]** 버튼을 클릭합니다.

### 3. 배포 완료
- 배포가 완료되면 Vercel에서 제공하는 URL(예: `pti-project.vercel.app`)을 통해 어디서든 접속이 가능합니다.
